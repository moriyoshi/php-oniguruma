--TEST--
Test onig_split() function : usage variations  - different parameter types for pattern
--SKIPIF--
<?php
...
foreach($inputs as $input) {
      echo "\n-- Iteration $iterator --\n";
      var_dump( onig_split($input, $string, $limit) );
      $iterator++;
};
fclose($fp);
echo "Done";
...
Warning: onig_split(): Empty regular expression in %s on line %d
array(1) {
  [0]=>
  string(13) "a b c d e f g"
}
...
Warning: onig_split() expects parameter 1 to be string, resource given in %s on line %d
NULL
Done
